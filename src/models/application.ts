...
import type { CurrentService } from '../util/device-service-details';
	...
	const exports = {
		...
		async get<T extends ODataOptionsWithoutCount<Application['Read']>>(
			...
			if (isId(slugOrUuidOrId)) {
				...
				})) as OptionsToResponse<Application['Read'], T, typeof slugOrUuidOrId>;
		...
		>(slugOrUuidOrId: string | number, options?: T) {
			...
			const app = await exports.get(
				slugOrUuidOrId,
				serviceOptions as typeof baseOptions,
			);
