...
const getReleaseModel = function (
	...
	const tagsModel = buildDependentResource(
		{ pine },
		{
			...
			async getResourceId(commitOrIdOrRawVersion): Promise<number> {
				const { id } = await exports.get(
	...
	// The CLI depends on this v format of exports so that it can monkey-patch short release commit support back in
	const exports = {
		/**
		 ...
		 * });
		 */
		async get<T extends ODataOptionsWithoutCount<Release['Read']>>(
		...
		>(
			...
			const rawRelease = await exports.get(
				commitOrIdOrRawVersion,
				mergePineOptions(
					baseReleaseOptions,
					options.release,
		 ...
		 * });
		 */
		async getLatestByApplication<
			T extends ODataOptionsWithoutCount<Release['Read']>,
		>(
			slugOrUuidOrId: string | number,
			options?: T,
		): Promise<OptionsToResponse<Release['Read'], T, undefined>[number]> {
			const [release] = (await exports.getAllByApplication(
				slugOrUuidOrId,
				mergePineOptions(
					{
						$top: 1,
		...
		async createFromUrl(
			...
			const { app_name, organization } =
				await sdkInstance.models.application.get(slugOrUuidOrId, appOptions);
		...
		async finalize(
			...
			const { id } = await exports.get(commitOrIdOrRawVersion, {
				$select: 'id',
			});
			await pine.patch({
				resource: 'release',
		...
		tags: {
			...
			>(
				...
				const release = await exports.get(commitOrIdOrRawVersion, {
					$select: 'id',
					$expand: {
