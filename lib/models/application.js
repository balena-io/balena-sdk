...
const getApplicationModel = function(deps, opts) {
	 ...
	 * });
	 */
	exports.getDashboardUrl = function(id) {
	 ...
	 * });
	 */
	exports.getAllWithDeviceServiceDetails = function(options, callback) {
		if (options == null) {
			options = {};
		}
		callback = findCallback(arguments);
		...
		const serviceOptions = mergePineOptions(
			{
				$expand: [{ owns__device: getCurrentServiceDetailsPineOptions(false) }],
			},
			options,
		);
	 ...
	 * });
	 */
	exports.get = function(nameOrSlugOrId, options, callback) {
		if (options == null) {
			options = {};
		}
		callback = findCallback(arguments);
	 ...
	 * });
	 */
	exports.getWithDeviceServiceDetails = function(
		nameOrSlugOrId,
		options,
		callback,
	) {
		...
		const serviceOptions = mergePineOptions(
			{
				$expand: [{ owns__device: getCurrentServiceDetailsPineOptions(true) }],
			},
			options,
		);
	 ...
	 * });
	 */
	exports.getAppByOwner = function(appName, owner, options, callback) {
		if (options == null) {
			options = {};
		}
		callback = findCallback(arguments);
	...
	exports.create = function(
		{ name, applicationType, deviceType, parent, organization },
		callback,
	) {
		callback = findCallback(arguments);
		...
		const deviceManifestPromise = deviceModel()
		...
		const organizationPromise = !organization
			? undefined
			: pine
					.get({
						...
						options: {
							$top: 1,
							$select: ['id'],
							$filter: { [isId(organization) ? 'id' : 'handle']: organization },
						},
					})
					.then(function([org]) {
						if (!org) {
		...
		return Promise.all([
			...
			.then(function([
				...
				if (applicationTypeId) {
					Object.assign(extraOptions, { application_type: applicationTypeId });
				}

				if (organizationId) {
					Object.assign(extraOptions, { organization: organizationId });
				}
	...
	exports.trackLatestRelease = (nameOrSlugOrId, callback) =>
		releaseModel()
			...
			.then(function(latestRelease) {
				...
				/** @type {object} */
				const app = latestRelease.belongs_to__application;
				return {
					applicationId: app.__id,
					commit: latestRelease.commit,
				};
			})
			.then(function({ applicationId, commit }) {
	 ...
	 * });
	 */
	exports.grantSupportAccess = function(
		nameOrSlugOrId,
		expiryTimestamp,
		callback,
	) {
