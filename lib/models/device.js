...
const getDeviceModel = function (deps, opts) {
	...
	const registerDevice = once(() =>
		require('balena-register-device')({ request }),
	);
	...
	const tagsModel = buildDependentResource(
		{ pine },
		{
			...
			getResourceId(uuidOrId) {
				return exports.get(uuidOrId, { $select: 'id' }).then(({ id }) => id);
			},
		},
	);
	...
	const exports = {
		...
		getMACAddresses: (uuidOrId) =>
			exports
				.get(uuidOrId, { $select: ['mac_address'] })
				.then(({ mac_address }) => {
					if (mac_address == null) {
						return [];
					}
		 ...
		 */
		generateUniqueKey: function (...args) {
			// Wrap with a function so that we can lazy load registerDevice
			return registerDevice().generateUniqueKey(...args);
		},
		...
		hasDeviceUrl: (uuidOrId) =>
			exports
				...
				.then(({ is_web_accessible }) => is_web_accessible),
		...
		_startOsUpdate: (uuid, targetOsVersion, skipCheck) =>
			Bluebird.try(function () {
				...
				.then(function (device) {
					...
					return osModel()
						.getSupportedVersions(device.is_of__device_type[0].slug)
						.then(function ({ versions: allVersions }) {
							if (
								!allVersions.some(
									(v) => bSemver.compare(v, targetOsVersion) === 0,
								)
							) {
								throw new errors.BalenaInvalidParameterError(
									'targetOsVersion',
		...
		startOsUpdate: (uuid, targetOsVersion) =>
			Bluebird.try(function () {
				...
				.then(function (device) {
					...
					return osModel().getSupportedVersions(
						device.is_of__device_type[0].slug,
					);
				})
				.then(function ({ versions: allVersions }) {
